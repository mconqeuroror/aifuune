...
export type EvilEyeProps = {
  ...
  /** Track cursor against viewport center (for fixed background layers). */
  ...
  useEffect(() => {
    ...
    function onMouseMove(e: Event) {
      ...
      if (globalMouseTracking) {
        ...
        mouse.tx = ((event.clientX - cx) / cx) * 0.85;
        mouse.ty = -(((event.clientY - cy) / cy) * 0.85);
        return;
      }
    ...
    const moveTarget = globalMouseTracking ? window : container;
